...
/*   Updated: 2025/01/08 16:58:24 by rojornod         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void place_map(mlx_t *mlx, char **map)
{
    ...
    mlx_texture_t *floor_texture = mlx_load_png("assets/snowfloor.png");
    mlx_texture_t *exit_texture = mlx_load_png("assets/exit.png");
    mlx_texture_t *collectible_texture = mlx_load_png("assets/collectible2.png");

    if (!wall_texture || !exit_texture || !floor_texture || !collectible_texture)
    {
...
{
    mlx_texture_t *player_texture = mlx_load_png("assets/santa.png");
    ...
    while (map[i] != NULL)
    {
        ...
        while (map[i][j] != '\0')
        {
            mlx_image_t *image = NULL;
            if (map[i][j] == 'P')
                image = mlx_texture_to_image(mlx, player_texture);
            if (image)
                mlx_image_to_window(mlx, image, j * TILE_SIZE, i * TILE_SIZE);
